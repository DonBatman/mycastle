...
core.register_node("mycastle:drawbridge_up",{
	description = "Drawbridge (Up)",
	...
	after_place_node = function(pos, placer, itemstack, pointed_thing)
		...
		for i, base_offset in ipairs(up_offsets) do
			...
			local success = core.set_node(target_pos, {name="mycastle:drawbridge_up2", param2 = par})
		end
	end,
	...
	on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
		if clicker and clicker:is_player() then
			...
			if can_lower then
				...
				core.swap_node(pos, {name="mycastle:drawbridge_down", param2 = par})

				for _, base_offset in ipairs(down_offsets) do
					local rotated_offset = get_rotated_offset(base_offset, par)
					local target_pos = vector.add(pos, rotated_offset)
					core.set_node(target_pos, {name="mycastle:drawbridge_down2", param2 = par})
				end
			end
		end
	end,
	...
	on_destruct = function(pos)
		...
		if node.name == "mycastle:drawbridge_up" then
			local par = node.param2
...
core.register_node("mycastle:drawbridge_down",{
	description = "Drawbridge (Down) 4x5",
	tiles = {"mycastle_wood.png"},
	drawtype = "nodebox",
	paramtype = "light",
	paramtype2 = "facedir",
		...
		if can_place then
			...
			if success then
				minetest.after(0, function()
					...
					if current_node.name == "mycastle:drawbridge_down" then
						...
						for i, base_offset in ipairs(down_offsets) do
							...
							local target_pos = vector.add(pos, rotated_offset)
							local part_place_success = core.set_node(target_pos, {name="mycastle:drawbridge_down2", param2 = current_par})
						end
					end
				end)
			end
	...
	after_place_node = function(pos, placer, itemstack, pointed_thing)
		...
		for i, base_offset in ipairs(down_offsets) do
			...
			local success = core.set_node(target_pos, {name="mycastle:drawbridge_down2", param2 = par})
		end
	end,
	...
	on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
		if clicker and clicker:is_player() then
			...
			if can_raise then
				...
				core.swap_node(pos, {name="mycastle:drawbridge_up", param2 = par})

				for _, base_offset in ipairs(up_offsets) do
					local rotated_offset = get_rotated_offset(base_offset, par)
					local target_pos = vector.add(pos, rotated_offset)
					core.set_node(target_pos, {name="mycastle:drawbridge_up2", param2 = par})
				end
			end
		end
	end,
	...
	on_destruct = function(pos)
		...
		if node.name == "mycastle:drawbridge_down" then
			local par = node.param2
...
core.register_node("mycastle:drawbridge_up2",{
...
core.register_node("mycastle:drawbridge_down2",{
...
core.register_craft({
	output = "mycastle:drawbridge_down 1",
	recipe = {
